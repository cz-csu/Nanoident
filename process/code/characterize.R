...
stifle <- foreach(model_name=list_model) %do% {
	...
	classification_results <- classify.detected.motifs.get_test_data.basic_group(difference_data,basecall, base_name, motif_center_summary, model, genome,genome_rev, 0, TRUE, iupac_nc, nb_threads)
