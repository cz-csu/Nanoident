...
/home/nanodisco/code/extract.R -i $path_fast5 -o $path_output -b $name_sample -p $nb_threads -s fq --basecall_version $basecall_version

# Check if some reads have been extracted
if [[ -f $path_output_sample".fastq" ]]; then
  if [[ $(wc -l < $path_output_sample".fastq") -eq 0 ]]; then
...
bwa mem -t $nb_threads -x ont2d $path_reference_genome $path_output_sample".fastq" 2> $path_output_sample"_log" | samtools view -b - | samtools sort -T $path_output_sample"_tmp" > $path_output_sample".sorted.bam"
