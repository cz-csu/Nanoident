...
option_list <- list(
	...
	make_option(c("--nb_peaks"), type="integer", default=1000, help="Number of sequence with p-value peaks to keep for each round (default is 2000)", metavar="<integer>"),
...
if(is.na(threshold)){
	output_fasta_name=paste0("motif_detection_motifnet/motifs_22bp_top_",nb_peaks,"_peaks.fasta")
	output_fasta_name_win=paste0("motif_detection_win_motifnet/motifs_22bp_top_",nb_peaks,"_peaks.fasta")
	output_bed_name=paste0("motif_detection/motifs_22bp_top_",nb_peaks,"_peaks.bed")
}else{
	output_fasta_name=paste0("motif_detection_motifnet/motifs_22bp_",paste0("th",threshold),"_",nb_peaks,"_peaks.fasta")
	output_fasta_name=paste0("motif_detection_win_motifnet/motifs_22bp_",paste0("th",threshold),"_",nb_peaks,"_peaks.fasta")
	output_bed_name=paste0("motif_detection/motifs_22bp_",paste0("th",threshold),"_",nb_peaks,"_peaks.bed")
}
...
detection_params <- list(
	...
	threshold=threshold, seq_params=seq_params, automated=automated, score_threshold=score_threshold, output_fasta_name=output_fasta_name,output_bed_name=output_bed_name,
	meme_main_dir="motif_detection_motifnet/", meme_analysis_dir=paste0("meme_",base_name,"/"), nbCPU=nb_threads
)
detection_params_win <- list(
	smooth_win_size=smooth_win_size, peak_win_size=peak_win_size, stat_val=stat_type, nb_peaks=nb_peaks, smooth_func=smooth_func,
	threshold=threshold, seq_params=seq_params, automated=automated, score_threshold=score_threshold, output_fasta_name=output_fasta_name_win,output_bed_name=output_bed_name,
	meme_main_dir="motif_detection_win_motifnet/", meme_analysis_dir=paste0("meme_",base_name,"/"), nbCPU=nb_threads
)
...
discovered_motifs <- wrapper.motif.detection(difference_data, genome, path_output, detection_params,detection_params_win, iupac_nc, list_motif) # list_motif can be ignore if NULL
