...
class SELayer1D(nn.Module):
    ...
    def forward(self, x):
        b, c, _ = x.size()
        #print(x.shape,self.avg_pool(x).shape)
        y = self.avg_pool(x)
        
        #print(y.shape)
        y = self.fc(y.view(b, c))
        # 将第二维度的每个元素重复32次，得到16x96维tensor
        y = y.repeat_interleave(32, dim=1)
        y=y.view(b, c, 1)
        z=y*x
        #x=y.expand_as(x)*x
        return z
class SelfAttentionPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        #pooled_output=pooled_output.view(b, c, 1)
...
class MotifNet3(nn.Module):
    ...
    def forward(self,mean):
        ...
        """
        col_w=self.po(mean_cat)
        sum_along_dim1 = col_w.sum(dim=1, keepdim=True)
        col_w= col_w / sum_along_dim1
        mean_cat=mean_cat*col_w
        """
...
class origin_dataset_train(Dataset):
    def __init__(self, transform,valid):
        ...
        if valid==False:     
            path_data = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_mean_12_code_filter_iso_offset_train3.pkl'
            path_data_basic_group = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_basic_group_12_code_filter_iso_offset_train3.pkl"
            path_data_base_num_fraction = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_base_num_fraction_12_code_filter_iso_offset_train3.pkl"
            path_data_std_diff = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_std_diff_12_code_filter_iso_offset_train3.pkl"
            path_data_time_diff = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_time_diff_12_code_filter_iso_offset_train3.pkl"
            ...
            path_test_avg_basequality = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_avg_basequality_12_code_filter_iso_offset_train3.pkl"
            path_y_basic_group = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/y_basic_group_12_code_filter_iso_offset_train3.pkl'
            path_y_offset = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/y_offset_12_code_filter_iso_offset_train3.pkl'
        else:
            path_data = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_mean_12_code_filter_iso_offset_valid3.pkl'
            path_data_basic_group = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_basic_group_12_code_filter_iso_offset_valid3.pkl"
            path_data_base_num_fraction = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_base_num_fraction_12_code_filter_iso_offset_valid3.pkl"
            path_data_std_diff = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_std_diff_12_code_filter_iso_offset_valid3.pkl"
            path_data_time_diff = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_time_diff_12_code_filter_iso_offset_valid3.pkl"
            ...
            path_test_avg_basequality = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_avg_basequality_12_code_filter_iso_offset_valid3.pkl"
            path_y_basic_group = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/y_basic_group_12_code_filter_iso_offset_valid3.pkl'
            path_y_offset = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/y_offset_12_code_filter_iso_offset_valid3.pkl'
            """
    ...
    def __getitem__(self, index):
        #data = torch.FloatTensor(data)
        return  self.data[index],self.data_base_num_fraction[index],self.data_std_diff[index],self.data_time_diff[index],self.data_avg_basequality[index],self.data_basic_group[index], self.label_basic_group[index],self.label_offset[index]
...
class origin_dataset_test(Dataset):
    def __init__(self, transform,motif,species):
        ...
        """
        if motif=="CCCRAG_-14":
            joblib.dump(self.data,"/home/nipeng/chenzheng/mulan-methyl/data_pkl/CCCRAG_mean.pkl")
        if motif=="RAACTC_-14":
            joblib.dump(self.data,"/home/nipeng/chenzheng/mulan-methyl/data_pkl/RAACTC_mean.pkl")
        """
        self.data = self.data.unsqueeze(1)
        self.data_base_num_fraction = self.data_base_num_fraction.unsqueeze(1)
        self.data_std_diff = self.data_std_diff.unsqueeze(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_workers', default=8, type=int, help='number of distributed processes')
    parser.add_argument('--is_distributed', default=False, type=bool) #False
    ...
    if args.is_distributed:
        ...
        data_loader_train = DataLoader(dataset_train,batch_sampler=train_batch_sampler,num_workers=args.num_workers,pin_memory=True)
        data_loader_valid = DataLoader(dataset_valid,sampler=val_sampler,num_workers=args.num_workers,batch_size=batch_size,pin_memory=True)
    else:
        data_loader_train = DataLoader(dataset_train,batch_size=batch_size,num_workers=args.num_workers,shuffle=True,pin_memory=True)
        data_loader_valid = DataLoader(dataset_valid,batch_size=batch_size,num_workers=args.num_workers,shuffle=True,pin_memory=True)
    """
    ...
    if args.is_distributed:
        model_basic_group = nn.parallel.DistributedDataParallel(model_basic_group) # device_ids will include all GPU devices by default
    ...
    criterion = FocalLoss()    #FocalLoss() nn.CrossEntropyLoss(label_smoothing=0.01) 
    ...
    for epoch in range(num_epochs):
        ...
        for index,data in enumerate(data_loader_train):
            mean,base_num_fraction,std,time,avg_basequality,basic,y_basic_group,y_offset=data
            mean=mean.to(device)
            std=std.to(device)
            time=time.to(device)
        ...
        with torch.no_grad():
            for index,Data in enumerate(data_loader_test):
                ...
                for b in range(outputs.shape[0]):
                    ...
                    if predicted_offset_basic==0 or predicted_offset_basic==1:
                        basic_type=1
                    else:
                        basic_type=0
                    for j in range(21):
                        if basic[b][17-predicted_offset[j]]==basic_type: #A or C #8-predicted_offset[j] self.data_basic_group=subet_test_data_basic_group.values[:,6:18]
