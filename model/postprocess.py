...
class MLP_basic(nn.Module):
    def __init__(self, input_size=22, hidden_size=128, num_classes=3):
        ...
        self.layers2 = nn.Sequential(nn.Linear(input_size,hidden_size),nn.ReLU(),nn.Linear(hidden_size, num_classes))
        self.layers3 = nn.Sequential(nn.Linear(input_size,hidden_size),nn.ReLU(),nn.Linear(hidden_size, num_classes))
        self.layers4 = nn.Sequential(nn.Linear(input_size,hidden_size),nn.ReLU(),nn.Linear(hidden_size, num_classes))
        self.layers5 = nn.Sequential(nn.Linear(input_size,hidden_size),nn.ReLU(),nn.Linear(hidden_size, num_classes))
        ...
        self.weights = nn.Parameter(torch.Tensor([0.284439705,0.193983631,0.101090158,0.084067644,0.047136005,0.068135741]))
    ...
    def forward(self,mean,base_num_fraction,g_num,avg_basequality,std,time):
        ...
        #g_num = self.layers6(g_num)
        #t_test_pval = self.layers6(t_test_pval)
...
class MotifNet(nn.Module):
    def __init__(self, input_size=22, hidden_size=128, num_classes=3):
        ...
        self.layers1 = nn.Sequential(nn.Conv1d(1, mid_channels, kernel_size=kernel_size, stride=1),nn.BatchNorm1d(mid_channels),nn.ReLU())
        ...
        self.layers11 = nn.Sequential(nn.Conv1d(1, mid_channels, kernel_size=kernel_size2, stride=1),nn.BatchNorm1d(mid_channels),nn.ReLU(),nn.Linear(mid_len2, mid_len),nn.BatchNorm1d(mid_channels),nn.ReLU())
        ...
        self.layers111 = nn.Sequential(nn.Conv1d(1, mid_channels, kernel_size=kernel_size3, stride=1),nn.BatchNorm1d(mid_channels),nn.ReLU(),nn.Linear(mid_len3, mid_len),nn.BatchNorm1d(mid_channels),nn.ReLU())
    ...
    def forward(self,mean,base_num_fraction,g_num,avg_basequality,std,time,t_test_pval):
        ...
        #g_num_3 = self.layers6(g_num)
        return self.weights[0]*mean4+self.weights[1]*base_num_fraction4+self.weights[2]*avg_basequality4+self.weights[3]*std4+self.weights[4]*time4##++self.weights[5]*g_num result1+result2+result3
class MLP(nn.Module):
    def __init__(self, input_size=22, hidden_size=128, num_classes=3):
        super(MLP, self).__init__()
        self.layers1 = nn.Sequential(nn.Conv1d(1, 32, kernel_size=3, stride=1),nn.BatchNorm1d(32),nn.ReLU(),Flatten(),nn.Linear(32*10, num_classes))
        self.layers2 = nn.Sequential(nn.Conv1d(1, 32, kernel_size=3, stride=1),nn.BatchNorm1d(32),nn.ReLU(),Flatten(),nn.Linear(32*10, num_classes))
        self.layers3 = nn.Sequential(nn.Conv1d(1, 32, kernel_size=3, stride=1),nn.BatchNorm1d(32),nn.ReLU(),Flatten(),nn.Linear(32*10, num_classes))
        self.layers4 = nn.Sequential(nn.Conv1d(1, 32, kernel_size=3, stride=1),nn.BatchNorm1d(32),nn.ReLU(),Flatten(),nn.Linear(32*10, num_classes))
        self.layers5 = nn.Sequential(nn.Conv1d(1, 32, kernel_size=3, stride=1),nn.BatchNorm1d(32),nn.ReLU(),Flatten(),nn.Linear(32*10, num_classes))
        """
        self.cls1=nn.Linear(32*10, num_classes)
        #self.layers1 = nn.Sequential(nn.Linear(input_size,hidden_size),nn.ReLU(),nn.Linear(hidden_size, num_classes))
        self.layers2 = nn.Sequential(nn.Linear(input_size,hidden_size),nn.ReLU(),nn.Linear(hidden_size, num_classes))
        self.layers3 = nn.Sequential(nn.Linear(input_size,hidden_size),nn.ReLU(),nn.Linear(hidden_size, num_classes))
        self.layers4 = nn.Sequential(nn.Linear(input_size,hidden_size),nn.ReLU(),nn.Linear(hidden_size, num_classes))
    ...
    def forward(self,mean,base_num_fraction,g_num,avg_basequality,std,time,t_test_pval):
        ...
        time = self.layers5(time)
        #g_num = self.layers6(g_num)
        return self.weights[0]*mean+self.weights[1]*base_num_fraction+self.weights[2]*avg_basequality+self.weights[3]*std+self.weights[4]*time#+self.weights[5]*g_num
...
class origin_dataset_train_basic(Dataset):
    def __init__(self, transform,valid):
        ...
        if valid==False:     
            path_data = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_mean_12_code_filter_iso_train.pkl'
            path_data_base_num_fraction = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_base_num_fraction_12_code_filter_iso_train.pkl"
            path_data_std_diff = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_std_diff_12_code_filter_iso_train.pkl"
            path_data_time_diff = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_time_diff_12_code_filter_iso_train.pkl"
            path_test_t_test_pval = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_t_test_pval_12_code_filter_iso_train.pkl"
            path_test_u_test_pval = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_u_test_pval_12_code_filter_iso_train.pkl"
            path_test_g_num = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_g_num_12_code_filter_iso_train.pkl"
            path_test_g_value = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_g_value_12_code_filter_iso_train.pkl"
            path_test_avg_basequality = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_avg_basequality_12_code_filter_iso_train.pkl"
            path_y_basic_group = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/y_basic_group_12_code_filter_iso_train.pkl'
            path_y_offset = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/y_offset_12_code_filter_iso_train.pkl'
...
class origin_dataset_train(Dataset):
    def __init__(self, transform,valid):
        ...
        if valid==False:     
            ...
            path_data = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_mean_12_code_filter_iso_offset_valid.pkl'
            path_data_base_num_fraction = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_base_num_fraction_12_code_filter_iso_offset_valid.pkl"
            path_data_std_diff = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_std_diff_12_code_filter_iso_offset_valid.pkl"
            path_data_time_diff = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_time_diff_12_code_filter_iso_offset_valid.pkl"
            ...
            path_test_avg_basequality = "/home/nipeng/chenzheng/mulan-methyl/data_pkl/data_avg_basequality_12_code_filter_iso_offset_valid.pkl"
            path_y_basic_group = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/y_basic_group_12_code_filter_iso_offset_valid.pkl'
            path_y_offset = '/home/nipeng/chenzheng/mulan-methyl/data_pkl/y_offset_12_code_filter_iso_offset_valid.pkl'
            """
...
if __name__ == '__main__':
    ...
    model = MotifNet(input_size=12, hidden_size=64, num_classes=21).to(device)
    ...
    model.load_state_dict(torch.load("/home/nipeng/chenzheng/mulan-methyl/my_model/mlp_15.pt")) #29,mlp_46_data_mean_12_code_filter_iso_offset_21_avg
    model_basic.load_state_dict(torch.load("/home/nipeng/chenzheng/mulan-methyl/my_model/mlp_71_data_mean_12_code_filter_iso_offset_3.pt")) #mlp_15_data_mean_12_code_filter_iso_offset_3_avg.pt
    ...
    for i,motif_item in enumerate(NO_motif):
        both_right=both_wrong=single21=addition3=0
        predict_result=[]
        ...
        with torch.no_grad():
            for index,Data in enumerate(data_loader_test):
                ...
                mean=mean.to(device)
                outputs=model(mean,base_num_fraction,g_num,avg_basequality,std,time,t_test_pval)
                outputs_basic=model_basic(mean_b,base_num_fraction_b,g_num,avg_basequality_b,std_b,time_b)
                #outputs_basic=model_basic(mean,base_num_fraction,g_num,avg_basequality,std,time)
                _, predicted_basic = torch.max(outputs_basic.data, 1)
                _, predicted_21 = torch.max(outputs.data, 1)
                predicted_21=predicted_21.cpu().numpy().tolist()
                predict_basic_prob=np.array(outputs_basic.data.cpu().numpy().tolist())
                row_mins = predict_basic_prob.min(axis=1)
                predict_basic_prob= (predict_basic_prob-row_mins[:, np.newaxis]) 
                row_sums = predict_basic_prob.sum(axis=1)
                predict_basic_prob=predict_basic_prob/ row_sums[:, np.newaxis] 
                ...
                for b in range(outputs.shape[0]):
                    ...
                    predicted_offset_basic=predicted[0]//7
                    ...
                    for j in range(21):
                        ...
                        if basic[b][17-predicted_offset[j]]==1 or basic[b][17-predicted_offset[j]]==0: #A or C #8-predicted_offset[j] self.data_basic_group=subet_test_data_basic_group.values[:,6:18]
                            ...
                            break
                    #predicted_offset_index=0
                    if predict_basic_prob[b][predicted_basic[b]]>2:
                        final=predicted_basic[b]*7+predicted_offset[predicted_offset_index]
                        #final=predicted_basic[b]+predicted_offset_basic*7
                    else:
                        #print(predicted_offset_basic,predicted_basic[b], predict_basic_prob[b][predicted_basic[b]])
                        final=torch.tensor(predicted_offset_basic).to('cuda')*7+predicted_offset[predicted_offset_index]
                        #final=torch.tensor(predicted_offset_basic).to('cuda')*7+predicted_offset[0]
                    #print(final)
                    final=final.cpu().item()
                    """
                    if predicted_21[b]%7==true_label_TP_offset[i] and predicted_basic[b]==true_label_TP_offset[i]:
                        both_right+=1
                    if predicted_21[b]%7!=true_label_TP_offset[i] and predicted_basic[b]==true_label_TP_offset[i]:
                        addition3+=1
                    if predicted_21[b]%7==true_label_TP_offset[i] and predicted_basic[b]!=true_label_TP_offset[i]:
        ...
        MEAN+=result[true_label_NO[i]]/len(dataset_test)
        print(most_num,true_label_NO[i],":",result[true_label_NO[i]]/len(dataset_test))
        #print(most_num,true_label_TP[i],":",result[true_label_TP[i]]/len(dataset_test),"both_right:",both_right/len(dataset_test),"both_wrong:",both_wrong/len(dataset_test),"single21:",single21/len(dataset_test),"addition3:",addition3/len(dataset_test))
    print(MEAN/4)
    #"""