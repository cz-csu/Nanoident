...
option_list <- list(
	...
	make_option(c("-a", "--IQR_factor"), type="double", default=1.5, help="IQR factor for outliers removal (0 to skip; smaller is harsher)", metavar="<double>"),
...
Sys.time()
#find the position of each read,and where fast5 file store,and dna squence
...
#sink("/home/nanodisco/difference_mid_result/index_wga2.txt")
#print(index_wga)
#sink()
Sys.time()
processed_reads_wga <- ""
...
final_stat_data <- foreach(idx_chunk=list_chunks, .combine=rbind) %:% when(handle.empty.chunk(idx_chunk, index_wga, index_nat, sample_name_wga, sample_name_nat, genome, chunk_size, path_output, inSilico, bc_version)) %do% {
	...
	#sink(paste0("/home/nanodisco/difference_mid_result/reads_wga_",idx_chunk,".txt"))
	#print(reads_wga)
	#sink()
	...
	#filter(model_mean>=0.001)
	#sink(paste0("/home/nanodisco/difference_mid_result/corrected_data_",idx_chunk,".txt"))
	...
	if(!is.null(corrected_data) | !is.null(nrow(prev_corrected_data))){ # If still data for chunk idx_chunk
		...
		gc()
		#sink(paste0("/home/nanodisco/difference_mid_result/stat_data_",idx_chunk,".txt"))
		#print(stat_data)
		#sink()
		...
		#sink(paste0("/home/nanodisco/difference_mid_result/processed_reads_wga_",idx_chunk,".txt"))
		#print(processed_reads_wga)
		#sink()
