...
prepare.index <- function(sample_name, path_input, path_output, genome, chunk_size, list_chunks){
	...
	#sink("/home/nanodisco/difference_mid_result/prepare.index.contigs_chunks2.txt")
	#print(contigs_chunks)
	#sink()
	...
	chunks_fast5_info <- foreach(chunk=list_chunks, .combine=rbind) %do% {
		...
		#sink("/home/nanodisco/difference_mid_result/prepare.index.bam2.txt")
		#print(bam)
		#sink()
		chunk_fast5_info <- as.data.frame(bam[[1]]) # Fail if no reads mapped to chunk
		#sink("/home/nanodisco/difference_mid_result/prepare.index.chunk_fast5_info2.txt")
		#print(chunk_fast5_info)
		#sink()
		if(nrow(chunk_fast5_info)>0){ # If mapped reads
			cov_th <- 1000
			detail_chunk_mapping <- generate.detail.chunk.mapping(chunk_fast5_info)
			#sink("/home/nanodisco/difference_mid_result/prepare.index.detail_chunk_mapping2.txt")
			#print(detail_chunk_mapping)
			#sink()
			extreme_coverage_region <- compute.chunk.coverage(detail_chunk_mapping) %>%
				filter(coverage>cov_th)
			#sink("/home/nanodisco/difference_mid_result/prepare.index.extreme_coverage_region2.txt")
			#print(extreme_coverage_region)
			#sink()
...
scoring.position <- function(sub_corrected_data, inSilico, inSilico_model, min_coverage){
	...
	sub_res <- sub_corrected_data %>%
		...
		summarize(means=list(norm_mean),stds=list(event_stdv),times=list(event_length),s_levels=list(standardized_level), .groups="drop_last") %>%
	...
	if((inSilico=="no" & nrow(sub_res)<2) | (inSilico!="no" & nrow(sub_res)==0)){ # When there is not enough values per dataset
		...
		res <- sub_res %>%
			mutate(data_type_stds=data_type,data_type_times=data_type,data_type_s_levels=data_type) %>%
			group_by(contig, position, dir, strand) %>%
			...
			mutate(mean_nat=mean(unlist(nat)),mean_wga=mean(unlist(wga))) %>%
			...
			dplyr::select(-c(wga, nat, reference_kmer)) %>%	 #这里原本有的
			spread(data_type_stds, stds) %>%
			mutate(std_diff=mean(unlist(nat))-mean(unlist(wga))) %>% 
			dplyr::select(-c(wga, nat)) %>%
			spread(data_type_times, times) %>%
			mutate(time_diff=mean(unlist(nat))-mean(unlist(wga))) %>% 
			dplyr::select(-c(wga, nat)) %>%
			spread(data_type_s_levels, s_levels) %>%
			mutate(s_level_diff=mean(unlist(nat))-mean(unlist(wga))) %>% 
			dplyr::select(-c(wga, nat)) 
		...
		res <- res %>%
			...
			#arrange(position) %>%
			mutate(mean_nat_around = map(row_number(), ~ mean_nat[pmax(.x - 3, 1):pmin(.x + 3, n())]))%>%
			mutate(mean_wga_around = map(row_number(), ~ mean_wga[pmax(.x - 3, 1):pmin(.x + 3, n())])) %>%
